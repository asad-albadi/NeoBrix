...
<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="docs/identity/shapes-dusk.png">
    <img src="docs/identity/shapes-dawn.png" alt="The Neobrix component vocabulary: card, button, chip, toggle, slider, progress meter, each with a chunky outline and a hard offset shadow">
  </picture>
</p>
...
Both strips are generated by `scripts/neobrix-generate-identity` from
`scripts/lib/palette.sh` — the same values the desktop itself uses — so they
cannot drift from the running shell.
